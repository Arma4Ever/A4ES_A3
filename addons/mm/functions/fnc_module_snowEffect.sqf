...
if(_mode == "init") then {
    ...
    if(!(_logic call FUNC(canExecuteModule))) exitWith {A3CS_LOGWARN("snowEffect: blokuje wykonanie modulu")true};

    _type = parseNumber (_logic getVariable ["type", "0"]);

    [_type] remoteExec [QFUNC(snowEffect), 0, true];
