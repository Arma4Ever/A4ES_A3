/*
 * Author: SzwedzikPL
 * forceGunLights module function
 */
...
if(_mode == "init") then {
    ...
    if(!(_logic call FUNC(canExecuteModule))) exitWith {A3CS_LOGWARN("forceGunLights: blokuje wykonanie modulu")true};

    private _units = (synchronizedObjects _logic) select {_x isKindOf "CAManBase"};

    {(group _x) enableGunLights "forceOn";} foreach _units;
