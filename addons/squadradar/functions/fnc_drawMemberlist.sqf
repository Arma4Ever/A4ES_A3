...
{
  private _color = (_x getVariable [QEGVAR(nametags,unitColor), [0, "#ffffff"]]) # 1;
  private _iconColor = _color;
  private _inRange = (_x distance2D ace_player) < RADAR_MAX_UNIT_DISTANCE;
  if (_inRange) then {
    _iconColor = (_x getVariable [QGVAR(iconColor), [0, _color]]) # 1;
  };
