...
if (_medic isEqualTo _patient) then {
  ["a4esserver_events_userFAKSelf", [_medic, _classname, _bodyPart]] call CBA_fnc_serverEvent;
} else {
  if (_patient getVariable ["a4es_common_isPlayer", false]) then {
    ["a4esserver_events_userFAK", [_medic, _patient, _classname, _bodyPart]] call CBA_fnc_serverEvent;
  } else {
    ["a4esserver_events_userFAKAI", [_medic, _classname, _bodyPart]] call CBA_fnc_serverEvent;
  };
};
