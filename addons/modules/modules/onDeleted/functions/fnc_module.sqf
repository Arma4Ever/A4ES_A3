...
private _logicVar = format [QGVAR(onDeleted_%1), _logic call BIS_fnc_netId];
missionNamespace setVariable [_logicVar, _logic];

TRACE_2("onDeleted - adding handler",_logicVar,_objects);

// Use waitUntilAndExecute because i can't reliably tell how much frames will pass until obj is removed
...
", _logicVar, QFUNC(onDeleted_moduleExec)];
