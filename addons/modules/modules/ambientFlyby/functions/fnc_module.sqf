...
if (_isActivated) then {
  ...
  private _proxy = createVehicle ["test_EmptyObjectForBubbles", [0,0,0],[], 0, "CAN_COLLIDE"];//Not perfect but way more stable than AI
  ...
  sleep (3 + _delay);
  [_posEnd,_plane,_proxy] spawn {
  params ["_posEnd","_plane","_proxy"];

  waitUntil {sleep 5; (_plane distance2D _posEnd) < 300};
