...
[{
    ...
    // EDIT
    // Close if in map preview
    if (a3cs_markers_currentMapPreview isNotEqualTo "") exitWith {
    ...
    if (_mouseOverType isEqualTo "marker") then {
        // EDIT
      // Disable markers editing
        if (((_marker find "_USER_DEFINED") isNotEqualTo -1) && ((markerShape _marker) isEqualTo "ICON")) exitWith {
            _display closeDisplay 2;
