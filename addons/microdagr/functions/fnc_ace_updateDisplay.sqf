...
case (APP_MODE_COMPASS): {
        ...
        private _compassAngleText = if (GVAR(settingUseMils)) then {
            [(floor ((6400 / 360) * (getDir ACE_player))), 4, 0] call CBA_fnc_formatNumber; //-- EDIT
        } else {
            ([(floor (getDir ACE_player)), 3, 1] call CBA_fnc_formatNumber) + "°" //degree symbol is in UTF-8 //-- EDIT
        };
...
case (APP_MODE_SETUP): {
        ...
        if (_markerId == 0) then {
            ...
            _settingListBox lbSetPictureRight [5, format ["a3\ui_f\data\map\Markers\NATO\%1.paa", _markerData # 1]];
